...
// ---------------------------------------------------------
// 👇 NAYA - backend ke validateCategoryPayload jaisa hi frontend check,
// taaki "Other" case me remark bhare bina hi request na jaaye
// ---------------------------------------------------------
...
const ProductFormModal = ({ isEditMode, onClose }) => {
    ...
    // 👇 NAYE - "Other" select hone par ye remarks bharne padte hai
    const [categoryRemark, setCategoryRemark] = useState("");
    const [subCategoryRemark, setSubCategoryRemark] = useState("");
    const [localError, setLocalError] = useState(""); // 👈 NAYA - submit se pehle ka validation error
    ...
    const [newVariants, setNewVariants] = useState(
        isEditMode ? [] : [{ ...emptyVariant(), isDefault: true }]
    );
    ...
    useEffect(() => {
        if (isEditMode && current) {
            ...
            // 👇 NAYA - agar categoryId null hai lekin categoryRemark hai, matlab "Other" tha
            setCategoryId(
                current.categoryId
                    ? String(current.categoryId)
                    : current.categoryRemark
                        ? "other"
                        : ""
            );
            setCategoryRemark(current.categoryRemark || "");

            setSubCategoryId(
                current.subCategoryId
                    ? String(current.subCategoryId)
                    : current.subCategoryRemark
                        ? "other"
                        : ""
            );
            setSubCategoryRemark(current.subCategoryRemark || "");
    ...
    // 👇 NAYE - "Other" flags, poore form me inhi se conditional UI decide hogi
    const isCategoryOther = categoryId === "other";
    const isSubCategoryOther = subCategoryId === "other";

    // ---------------- Category / SubCategory handlers ----------------
    // 👇 NAYA - category "Other" select hote hi subCategory bhi automatically "other" ho jaati hai
    const handleCategoryChange = (value) => {
        setCategoryId(value);
        if (value === "other") {
            setSubCategoryId("other");
        } else {
            setSubCategoryId("");
            setCategoryRemark("");
        }
    ...
    const handleSubmit = async (e) => {
        ...
        // 👇 NAYA - request bhejne se pehle hi "Other" wala remark validation
        ...
        // 👇 categoryId/subCategoryId "other" bhi ho sakta hai - waisa hi bhej rahe hai,
        // backend usko Number() nahi karega jab "other" hoga
        formData.append("categoryId", categoryId);
        formData.append("subCategoryId", subCategoryId);

        // 👇 NAYE - remarks (backend sirf "other" case me inhe zaroori maanta hai)
        formData.append("categoryRemark", categoryRemark);
        formData.append("subCategoryRemark", subCategoryRemark);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl">
                ...
                {/* 👇 NAYA - "Other" remark validation ka error yaha dikhega */}
                {localError && (
                    <p className="mb-3 rounded-md bg-red-50 px-3 py-2 text-sm text-red-600">
                        {localError}
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                // 👇 NAYA - category "Other" hai toh subCategory forcefully "other" hai, edit karne ki zaroorat nahi
                                disabled={!selectedCategory && !isCategoryOther}
                            ...
                            >
                                ...
                                {/* 👇 NAYA - category "Other" hai toh sirf yahi option milega, warna list ke end me extra option ke roop me */}
                                <option value="other">Other</option>
                            </select>
                        </div>

                        {/* 👇 NAYA - category "Other" hai tabhi ye remark dikhega, required hai */}
                        {isCategoryOther && (
                            <div className="col-span-2">
                                <label className="mb-1 block text-sm text-stone-600">
                                    Category Remark <span className="text-red-500">*</span>
                                </label>
                                <input
                                    required
                        ...
                        {/* 👇 NAYA - category Other ho ya sirf subCategory Other ho, dono case me dikhega, required hai */}
                        {(isCategoryOther || isSubCategoryOther) && (
                            <div className="col-span-2">
                                <label className="mb-1 block text-sm text-stone-600">
                                    Sub Category Remark <span className="text-red-500">*</span>
                                </label>
                                <input
                                    required
                                    value={subCategoryRemark}
                                    onChange={(e) => setSubCategoryRemark(e.target.value)}
                                    placeholder={
                                        categoryId == "other"
                                            ? "laptop"
                                            : selectedCategory?.subCategories?.[0]?.productSubCategoryName || "speaker"
                                    }
                                    className="w-full rounded-md border border-stone-300 px-3 py-2 text-sm"
                                />
                            </div>
