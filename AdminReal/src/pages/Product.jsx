...
const Product = () => {
    ...
    //ACTIVE AUR DEACTIVATE PRODUCTS
    const handleToggleDelete = async (product) => {
        try {
            setActionLoading(true);
            const res = await api.put(`/api/product/delete/${product.id}`);
            if (res.data.success) {
                // Refresh list after successful toggle
                dispatch(fetchAllProductsAdmin({
                    page, limit: 10, status,
            ...
            alert(error.response?.data?.message || "Failed to update product delete status");
        } finally {
            setActionLoading(false);
        }
    };
                                ...
                                <div className="flex items-center gap-6 text-center">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            className={`text-xs px-2.5 py-1 rounded-full font-medium transition ${!product.isDelete
                                                ? "bg-red-500 text-white hover:bg-red-900"
                                                : "bg-green-500 text-white hover:bg-green-900"
                                                }`}
                                            title="Click to toggle delete status"
                                        >
                                            {!product.isDelete ? "Delete" : "Restore"}
                                        </button>
