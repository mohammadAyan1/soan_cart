...
export const fetchAllProductsAdmin = createAsyncThunk(
    ...
    async (
        ...
        try {
            ...
            if (isApprove !== "") params.isApprove = isApprove;
            if (fromDate) params.fromDate = fromDate;
            if (toDate) params.toDate = toDate;
            if (sortBy) params.sortBy = sortBy;
            if (hasOrdered) params.hasOrdered = true;
            if (hasCart) params.hasCart = true;
            if (hasWishlist) params.hasWishlist = true;
            if (hasReview) params.hasReview = true;
            const res = await api.get("/api/product/getall/admin", { params });
            return res.data; // { success, message, data, pagination, filters }
        } catch (error) {
            return rejectWithValue(
                error.response?.data?.message || "Admin products load nahi ho paaye"
            );
...
const productSlice = createSlice({
    ...
    initialState: {
        ...
        adminFilters: null, // 👈 NAYA - { vendors, categories, subCategories }
        adminLoading: false,
        adminError: null,
    },
    reducers: {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchAllProductsAdmin.fulfilled, (state, action) => {
                ...
                state.adminFilters = action.payload.filters || null; // 👈 NAYA
...
export const { clearCurrentProduct, clearProductError, clearAdminProductError } = productSlice.actions;
