...
import { useProductListTracker } from "../../hooks/useProductListTracker"; // 👈 Scroll & Impression tracker hook
...
export default function CategoryDetailScreen() {
    ...
    // 👇 Scroll & Impression Tracker Hook integration
    ...
    // 👇 subcategory pe click -> product-list page
    const handleSubCategoryPress = (sub) => {
        trackEvent({
            eventType: "SUB_CATEGORY_CLICK",
            screen: SCREEN_NAME,
    ...
    if (loading && items.length === 0) {
        return (
            <View style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#fff", paddingTop: insets.top }}>
                <ActivityIndicator size="small" color="#16a34a" />
            </View>
        );
    }
    ...
    if (error && items.length === 0) {
        return (
            <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 24, backgroundColor: "#fff", paddingTop: insets.top }}>
                <Text style={{ color: "#DC2626", textAlign: "center" }}>{error}</Text>
            </View>
        );
    }
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "#fff", paddingTop: insets.top }}>
            ...
            <View style={{
                ...
                <Text style={{ fontSize: 17, fontWeight: '700', color: '#1A1D23' }}>
                    {title || "Category"}
                </Text>
                <View style={{ width: 24 }} />
            </View>
            ...
            <FlatList
                ...
                onEndReached={onEndReached}
                onEndReachedThreshold={0.4}
                onScroll={trackScroll} // 👈 Scroll tracking event yahan hit hoga
                scrollEventThrottle={16}
                onViewableItemsChanged={onViewableItemsChanged} // 👈 Product impressions yahan track honge
